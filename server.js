...
app.get("/get-user-score/:username", async (req, res) => {
  ...
  try {
      ...
      // ✅ NEW: Prevent coupon redemption if reward points < 50
      if (userPoints < 50) {
          return res.json({ score: userPoints, message: "⚠ You need at least 50 points to redeem a coupon." });
      }
...
app.get("/listings/business", verifyToken, async (req, res) => {
  try {
    ...
    console.error(`❌ Error fetching claimed listings for ${req.user?.username}:`, error);
